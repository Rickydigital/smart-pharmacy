...
class StockAdjustmentController extends Controller implements HasMiddleware
{
    ...
    public function searchInventory(Request $request): JsonResponse
    {
        ...
        $items = $inventories->map(fn (Inventory $inventory) => [
    ...
    public function store(Request $request, SystemNotificationService $notifier): RedirectResponse
    {
        ...
        $stockAdjustment = DB::transaction(function () use ($pharmacy, $validated) {
        ...
        $notifier->notifyStockAdjustmentCreated($stockAdjustment);
    ...
    public function reject(Request $request, StockAdjustment $stockAdjustment): RedirectResponse
    {
        ...
        if (! $stockAdjustment->isDraft()) {
            return back()->with('error', 'Only draft adjustments can be rejected.');
        }
    ...
    public function cancel(StockAdjustment $stockAdjustment): RedirectResponse
    {
        ...
        if (! $stockAdjustment->isDraft()) {
            return back()->with('error', 'Only draft adjustments can be cancelled.');
        }
