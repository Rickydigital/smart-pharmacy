...
use App\Services\MarketOrderSaleService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

class MarketLocalOrderController extends ApiController
{
    public function searchProducts(Request $request): JsonResponse
    {
        ...
        $products = Product::query()
            ->with(['category:id,name', 'baseUnit:id,name,code', 'productUnits.unit'])
                    ...
                    'units' => $product->productUnits
                        ...
                        ->map(function ($productUnit) {
                            return [
                                ...
                                'can_sell_wholesale' => (bool) $productUnit->can_sell_wholesale,
    ...
    public function createSupplierSale(
        Request $request,
        MarketOrderSaleService $saleService,
        ...
        $validated = $request->validate([
            ...
            'payment_method' => ['nullable', 'string', 'in:credit,cash,mobile_money,bank_transfer'],
        ...
        $paymentMethod = $validated['payment_method']
            ?? data_get($validated, 'central_order.payment_method')
            ?? 'credit';

        $paidAmount = (float) ($validated['paid_amount']
            ?? data_get($validated, 'central_order.paid_amount')
            ?? 0);

        $sale = $saleService->createSaleFromMarketOrder(
        ...
        $nextStatus = $sale->payment_status === 'paid'
            ? 'paid'
            : 'ready_for_payment';
        ...
        $marketBridge->updateStatus(
            ...
            status: $nextStatus,
            note: 'Supplier created local sale #' . $sale->sale_no,
        );

        return $this->success([
            'message' => 'Local sale created from market order.',
            'sale' => $sale,
            'central_order_status' => $nextStatus,
        ]);
    }
}